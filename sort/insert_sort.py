...
注意到左边是从小到大的已排序状态，所以如果该key（未排序的第一个元素）小于左边的j值（左边最大值），那么我们让左边最大值右移一位，并且递减j值，让左边第二大与key值比较，以此类推，假使该key值真的很小，甚至比左边已排序的最小元素都小，那么此时j值已经递减到-1了，且所有左边元素都已经右移一位，那么此时令arr[j+1]即arr[0]为key，比较结束，再进行下一次循环，直到数组的已排序占据整个数组。
'''
'''
